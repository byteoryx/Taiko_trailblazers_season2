...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            withdraw_from_taiko_to_recipient_chains_executor(
                sql=sql, today_bridge=today_table_name, total_accs=total_accs
            )
            transfer_from_recipient_chains_to_cex_executor(
                sql=sql, total_accs=total_accs
            )

            logger.info(f"withdraw balance from taiko to cex circle is completed.")
