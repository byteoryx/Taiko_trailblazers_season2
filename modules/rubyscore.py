...
def rubyscore_vote(
    ...
    if old_balance.float > 0.00001:
        ...
        if tx_hash:
            ...
            logger.info(f'#{index} | {account.address}: rubyscore | {taiko_chain.explorer}/{tx_hash} | {status}.')
        else:
            logger.error(f'#{index} | {account.address}: rubyscore vote tx has failed.')
    else:
        logger.warning(f'#{index} | {account.address}: rubyscore | {old_balance.float} $ETH on {taiko_chain.name}, '
