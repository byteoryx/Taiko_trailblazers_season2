...
from settings.config import leave_on_source, sleep_between_txs_in_sec
...
def ritsu_single_swap(
    ...
    old_source_balance = get_balance_of(address=account.address, contract=source_token.address) \
        if source_token.ticker != 'ETH' else get_balance(address=account.address, rpc=taiko_chain.rpc)

    old_destination_balance = get_balance_of(address=account.address, contract=destination_token.address) \
