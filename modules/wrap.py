...
def wrap_main(
    ...
    minimum_required_balance = 0.00001
    ...
    if not unwrap_only:
        if balance.float > minimum_required_balance:
            ...
            if wrap:
                logger.info(f'#{index} | {account.address}: wrap | {taiko_chain.explorer}/{wrap}')
                sleep_in_range(sec_from=60 + sleep_between_txs_in_sec[0], sec_to=60 + sleep_between_txs_in_sec[1])
            else:
                logger.error(f'#{index} | {account.address}: wrap tx has failed.')
        else:
    ...
    if weth_balance.int:
        ...
        if tx:
            logger.info(f'#{index} | {account.address}: unwrap | {taiko_chain.explorer}/{tx}')
        else:
            logger.error(f'#{index} | {account.address}: unwrap tx has failed.')
    else:
