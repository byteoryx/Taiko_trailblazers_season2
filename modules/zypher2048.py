...
def zypher2048_main(
    ...
    if old_balance.float > 0.001:
        ...
        if tx_hash:
            ...
            logger.info(f'#{index} | {account.address}: zypher2048 | {taiko_chain.explorer}/{tx_hash} | {status}.')
        else:
            logger.error(f'#{index} | {account.address}: zypher2048 tx has failed.')
    else:
        logger.warning(f'#{index} | {account.address}: zypher2048 | {old_balance.float} $ETH on {taiko_chain.name}, '
