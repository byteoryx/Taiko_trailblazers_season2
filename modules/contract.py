...
from tools.crypto import get_balance, contract_deploy_tx
...
def contract_deploy(
    ...
    if old_balance.float > 0.0001:
        ...
        if tx_hash:
            ...
            logger.info(f'#{index} | {account.address}: contract_deploy | {taiko_chain.explorer}/{tx_hash} | {status}.')
        else:
            logger.error(f'#{index} | {account.address}: contract_deploy tx has failed.')
    else:
        logger.warning(
            f'#{index} | {account.address}: contract_deploy | {old_balance.float} $ETH on {taiko_chain.name}, '
