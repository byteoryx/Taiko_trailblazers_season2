...
from tools.crypto import get_balance, get_balance_of, meridian_approve_tx, meridian_usdc_deposit_tx
    ...
    if old_balance.float > minimum_balance_required:
        ...
        if approve_hash:
            ...
            if tx_hash:
                ...
                logger.info(
                    f'#{acc.id} | {account.address}: meridian_deposit | {taiko_chain.explorer}/{tx_hash} | {status}.')
            else:
